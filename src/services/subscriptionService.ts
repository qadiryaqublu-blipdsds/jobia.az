...
export const SUBSCRIPTION_PLANS: SubscriptionPlan[] = [
  ...
  // -------------------------------------------------------------
  {
    ...
    features: [
      ...
      'Müraciət statusları və Google Chat əlaqəsi',
...
// Initial pre-configured subscriptions
const INITIAL_SUBSCRIPTIONS: UserSubscription[] = [
  {
    id: 'sub-sample-1',
    userId: 'user-employer-1',
    userEmail: 'hr@kapitalbank.az',
    userName: 'Kapital Bank (Nərgiz Məmmədova)',
    role: 'business',
    planId: 'plan-employer-pro',
    tier: 'PRO',
    status: 'ACTIVE',
  ...
  },
  {
    ...
    userEmail: 'samir.aliyev@mail.az',
    userName: 'Samir Əliyev',
    role: 'candidate',
    planId: 'plan-candidate-premium',
    tier: 'PREMIUM',
    status: 'ACTIVE',
    billingCycle: 'yearly',
...
const INITIAL_TRANSACTIONS: PaymentTransaction[] = [
  {
    ...
    userId: 'user-employer-1',
    userEmail: 'hr@kapitalbank.az',
    userName: 'Kapital Bank HR',
    subscriptionId: 'sub-sample-1',
    planName: 'Pro Recruiter Planı',
    amount: 49,
    currency: 'AZN',
    status: 'SUCCESS',
    paymentMethod: 'Bank Kartı (Birbank Visa)',
    cardLast4: '4128',
    transactionDate: '2026-08-01T10:15:30.000Z',
  },
  {
    id: 'pay-tx-1002',
    userId: 'user-candidate-1',
    userEmail: 'samir.aliyev@mail.az',
    userName: 'Samir Əliyev',
    subscriptionId: 'sub-sample-2',
    planName: 'Candidate Premium AI Planı (İllik)',
    amount: 82.8,
    currency: 'AZN',
    status: 'SUCCESS',
    paymentMethod: 'Bank Kartı (Mastercard)',
    cardLast4: '7844',
    transactionDate: '2026-07-15T14:22:10.000Z',
  },
];
...
export function getStoredSubscriptions(): UserSubscription[] {
  ...
  if (!raw) {
    localStorage.setItem(SUBSCRIPTION_STORAGE_KEY, JSON.stringify(INITIAL_SUBSCRIPTIONS));
    return INITIAL_SUBSCRIPTIONS;
  }
  try {
    return JSON.parse(raw) || INITIAL_SUBSCRIPTIONS;
  } catch {
    return INITIAL_SUBSCRIPTIONS;
  }
}
...
export function getStoredTransactions(): PaymentTransaction[] {
  ...
  try {
    return JSON.parse(raw) || INITIAL_TRANSACTIONS;
  } catch {
    return INITIAL_TRANSACTIONS;
  }
}
...
export function getPlansByRole(role: UserRole): SubscriptionPlan[] {
  ...
  const targetRole = role === 'business' ? 'business' : 'candidate';
  return SUBSCRIPTION_PLANS.filter((p) => p.role === targetRole);
}
...
export function applySubscriptionUpgrade(data: {
  ...
  // Replace or add user active sub
